...
@section('content')
<div class="container">
    <h2 class="mb-4">Pagos Eliminados</h2>
    ...
    @else
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $pago->estudiante->nombre ?? 'No registrado' }}</td>
                    <td>{{ $pago->tipo }}</td>
...
.container {
    padding-top: 60px; /* separa la card del borde superior */
}
...
.eliminados-header {
    background-color: #3366cc; /* azul intenso */
    color: white;
    padding: 10px 15px;
    border-radius: 5px;
    display: flex;
...
.table-eliminados th {
    background-color: #6699ff; /* azul medio */
    color: white;
    padding: 8px;
    text-align: left;
}
...
/* Filas de la tabla */
.table-eliminados td {
    background-color: #cce0ff; /* azul muy claro */
    padding: 8px;
}

/* Botón Restaurar */
.btn-restaurar {
    background-color: #0052cc; /* azul oscuro */
    color: white;
    border: none;
    padding: 5px 10px;
    border-radius: 4px;
...
.btn-restaurar:hover {
    background-color: #003d99; /* azul más oscuro al pasar el mouse */
}
                            ...
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('pagos.restaurar', $pago->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn-restaurar" onclick="return confirm('¿Restaurar este pago?')">
